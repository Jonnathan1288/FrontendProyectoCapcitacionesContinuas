...
export class AsignacionRolCapacitadorComponent implements OnInit {
	...
	public listDocentesCapacitadores() {
		this.capacitadorService.findByAllPaginator(0, 5, ["idCapacitador", "desc"]).subscribe({
			next: (resp: any) => {
				this.toastrService.info('', 'CAPACITADORES OBTENIDOS');
				this.listClassCapacitador = resp.content;
				this.listDocentesCapacitadoresFilter = this.listClassCapacitador;
	...
	//NUEVOS CAMBIOS EN LA PARTE CON PAGINATOR
	public options = [
		{ label: '5', value: 5 },
	];
	...
	public getPaginatorCapacitadores(size: any) {

		this.capacitadorService.findByAllPaginator(0, size, ["idCapacitador", "desc"]).subscribe({
			next: (resp: any) => {
				this.toastrService.info('', 'TAMAÑO DE CAPACITADORES SOLICITADOS: ' + size);
				this.listClassCapacitador = resp.content;
				this.listDocentesCapacitadoresFilter = this.listClassCapacitador;
			}, error: (err) => {
				this.toastrService.error('', 'INCONVENIENTE AL OBTENER CAPACITADORES');
			}
		});
	}
